...
def notears_linear(X, y, lambda1, loss_type, max_iter=100, h_tol=1e-8, rho_max=1e+16, w_threshold=0.3):
    ...
    def _func(w):
        ...
        h, G_h = _h(W)
        g_x = _g(X_, W, y, beta)
        obj = loss + 0.5 * rho * h * h + 0.5 * rho * g_x * g_x + alpha * h + gamma * g_x + lambda1 * w.sum()
        G_smooth = G_loss + (rho * h + alpha) * G_h
        g_obj = np.concatenate((G_smooth + lambda1, - G_smooth + lambda1), axis=None)
        return obj, g_obj

    n, d = X.shape
    w_est, rho, alpha, gamma, h = np.zeros(2 * d * d), 1.0, 0.0, 0.0, np.inf  # double w_est into (w_pos, w_neg)
